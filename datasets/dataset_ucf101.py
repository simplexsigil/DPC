    ...
    def __init__(self,
        ...
        if mode == 'train':
            ...
            split = os.path.expanduser(
                '~/datasets/UCF101/split/test_split%02d.csv' % self.which_split)  # use test for val, temporary
            video_info = pd.read_csv(split, header=None)
        ...
        if mode == 'val': self.video_info = self.video_info.sample(frac=0.3)  # TODO: This makes no sense with splits.
