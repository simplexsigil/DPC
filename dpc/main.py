...
from dataset_kinetics import Kinetics400Dataset, KineticsDatasetUtils
...
# /mnt/athene/datasets/kinetics/kinetics400-skeleton/skele-motion
...
def main():
    ...
    multi_transforms = prepare_augmentations(augmentation_settings, args)

    writer_train, writer_val = get_summary_writers(args.img_path, args.prefix)

    write_settings_file(args, exp_path)

    train_loader, train_len = get_data(multi_transforms, 'train', args, augmentation_settings)
    val_loader, val_len = get_data(multi_transforms, 'val', args, augmentation_settings)
...
def prepare_augmentations(augmentation_settings, args):
    ...
    if args.img_dims is None:
        multi_transforms.append(transforms.Compose([
        ...
        for dim in args.img_dims:
            multi_transforms.append(transforms.Compose([
                RandomRotation(degree=augmentation_settings["rot_range"]),
                RandomSizedCrop(size=dim, crop_area=augmentation_settings["crop_arr_range"],
                                consistent=True),
                ColorJitter(brightness=augmentation_settings["val_range"], contrast=0,
                            saturation=augmentation_settings["sat_range"],
                            hue=[val / 360. for val in augmentation_settings["hue_range"]]),
...
def get_data(multi_transforms, mode='train', args=None, augmentation_settings=None, random_state=42):
    if not args.use_dali or mode == "val":
        if args.dataset == 'kinetics400':
            dataset = Kinetics400Dataset(split=mode,
                                         ...
                                         time_shifts=args.multi_dims_time_shifts,
                                         seq_len=args.seq_len,
        ...
        data_loader = data.DataLoader(dataset,
                                      ...
                                      collate_fn=KineticsDatasetUtils.kinetics_collate,
                                      sampler=sampler,
                                      shuffle=False,
